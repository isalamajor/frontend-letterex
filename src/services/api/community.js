...
const API_URL = process.env.NEXT_PUBLIC_API_BASE_URL + "/community";
...
const getCommunities = async (language) => {
  try {
    ...
    return {
      ok: false,
      error: "An error occurred when fetching the communities",
    };
  }
};
...
const requestMembership = async (communityId) => {
  try {
    ...
    return {
      ok: false,
      error: "An error occurred when sending community request",
    };
  }
};
