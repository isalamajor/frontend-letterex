      ...
      {showExplorePage ? (
        <div className="text-gray-900 p-2 md:p-10 rounded-tl-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 flex flex-col gap-2 flex-1 w-full h-full">
          <div className="flex  items-center flex-col mb-5">
            ...
            <button onClick={() => setShowExplorePage(!showExplorePage)}>
              click
            </button>

            <p>It seems that you're not part of a community yet...</p>
            <p className="text-gray-500 text-center md:w-[50%]">
              The members of a community can post letters, suggest topics to
              write about, and chat! Join a community or create your own one and
              try it out!
            </p>
          </div>
...
const CommunityBoard: React.FC<CommunityBoardProps> = ({
  ...
  const [communitySelected, setCommunitySelected] = useState<Community | null>(
    null,
  );
    ...
    <div className="text-gray-900 p-2 md:p-10 rounded-tl-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 flex flex-col gap-2 flex-1 w-full h-full">
      ...
      <div className="flex justify-between gap-2">
        ...
        <div className="flex flex-row gap-2">
          <button
            className="bg-green-400 hover:bg-green-500 self-right text-white rounded-sm p-2 flex flex-row items-center gap-2"
            onClick={() => goToExplore()}
          >
            <Telescope />
            Explore
                ...
                {sectionSelected === 0 ? (
                  <SuggestedTopics
                    ...
                    onTopicDeleted={(topicId) => {
                      const topicsUpdated = suggestedTopics.filter(
                        (t) => t.id != topicId,
                      );
                      setSuggestedTopics([...topicsUpdated]);
                    }}
      ...
      {communitySelected && (
        <MemberList
          communityId={communitySelected.id}
          toggleShowAll={(state) => setShowMembers(state)}
        />
      )}
    </div>
  );
...
const CommunityCard: React.FC<CommunityCardProps> = ({
  ...
  return (
    <div
      className="items-center px-8 py-4 rounded-lg bg-white border border-gray-200  hover:border-orange-200 shadow-md w-full flex flex-row justify-between gap-8"
      key={id}
    >
      ...
      <div className="flex h-30 items-start">
        {userSentRequestUpdated ? (
          <p className="text-green-500 flex flex-row gap-1">
            <Check />
            Request sent
          </p>
        ) : (
          <button
            className="bg-green-500 px-4 py-1 rounded-md text-white cursor-pointer hover:bg-green-600 flex align-bottom"
            onClick={sendRequest}
          >
            Join
          </button>
...
const MemberCard = ({ user }: { user: Friend }) => {
  ...
  return (
    <div
      className="p-4 rounded-lg bg-white hover:bg-gradient-to-r hover:from-red-100 hover:via-orange-100 hover:to-yellow-100 border border-gray-200 shadow-md flex flex-row gap-3 items-center"
      onClick={goToProfile()}
    >
      <img
...
const MemberList = ({
  ...
  communityId: string;
  toggleShowAll: (state: boolean) => void;
}) => {
        ...
        {showAll ? (
          <div className="mt-5">
            <div className="flex flex-row justify-between text-gray-500 mb-1">
              <div
                className="hover:underline cursor-pointer flex flex-row gap-1 items-center"
                onClick={() => {
                  setItemsPerPage(6);
                  setPage(0);
                  setShowAll(false);
          ...
          <>
            ...
            <div className="w-full h-fit flex flex-row gap-2">
              {!showAll && totalMembers > itemsPerPage && page > 0 && (
                <div
                  className="p-2 rounded-lg bg-white text-clip hover:bg-gray-100 border border-gray-200 shadow-md flex justify-center items-center"
...
const CommunityList = () => {
  ...
  return (
    <>
            ...
            <SelectContent>
              ...
              {languages.map((lang) => (
                <SelectItem key={lang.name} value={lang.name}>
                  <div className="flex flex-row gap-2">
                    <img src={lang.image} className="h-5 w-5"></img>
                    {lang.name}
                  </div>
                </SelectItem>
              ))}
      ...
      {communities.length === 0 ? (
      ...
      ) : communities.filter((c) => c.name.toLowerCase().includes(searchFilter))
          .length === 0 ? (
        <div className=" bg-gray-100 rounded-lg my-5 min-h-[55vh] flex flex-row gap-2 justify-center items-center">
        ...
        <div className="flex flex-col justify-between">
              ...
              .map((com) => (
                <div key={com.id}>
                  <CommunityCard
                    ...
                    members={com.members}
                    letters={com.lettersShared}
                    language={com.language}
                    isPrivate={com.isPrivate}
                    userIsMember={com.userIsMember}
                    userSentRequest={com.userSentRequest}
                  />
                </div>
              ))}
          </div>
          {communities.length > ITEMS_PER_PAGE && (
            <Pagination
              count={Math.ceil(communities.length / ITEMS_PER_PAGE)}
              variant="outlined"
              shape="rounded"
...
const CreateCommunityForm: React.FC<CreateCommunityFormProps> = ({
  ...
  const stylesInput =
    "w-full p-2 bg-gray-100 border-1 border-gray-300 rounded-sm focus:border-orange-300 focus:ring-yellow-200 focus:ring-1 outline-none w-[50%]";
  const errorStylesInput =
    "w-full p-2 bg-gray-100 border-1 border-red-500 rounded-sm focus:border-red-500 focus:ring-red-200 focus:ring-1 outline-none w-[50%]";
  ...
  const createCommunityOnClick = async () => {
    ...
    if (description.length < 10) {
      setInvalidDescription(
        "Description is too short. Take at least 10 characters.",
      );
    } else {
    ...
    <div className="flex flex-col gap-3">
      <div className="flex flex-col">
        ...
        <input
          ...
          className={invalidName ? errorStylesInput : stylesInput}
          value={communityName}
          onChange={(e) => {
            setInvalidName("");
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="col-span-2 flex flex-col">
          ...
          <textarea
            ...
            className={
              (invalidDescription ? errorStylesInput : stylesInput) +
      ...
      <div className="flex flex-col gap-0">
        <label className="flex gap-2">
          <input
            ...
            name="option"
            value="public"
            checked={publicCommunity}
            onChange={() => setPublicCommunity(!publicCommunity)}
          />
      ...
      <button
        className="bg-green-500 hover:bg-green-600 self-right text-white rounded-sm p-2"
  ...
  return (
    <div className="h-[35rem] w-full md:w-[70%] border-15 border-gray-200 rounded-sm z-index-2 bg-gray-100 shadow-xl">
      ...
      <div className="w-full flex justify-end">
        {addingNew ? (
          <MessageCircleX
            className="bg-white text-red-400 ring-gray-400 ring-2 text-gray-900 rounded-sm h-10 w-10 p-1 mr-5"
            onClick={() => setAddingNew(false)}
          ></MessageCircleX>
        ) : (
          <MessageCirclePlus
            className="bg-white hover:text-yellow-400 ring-gray-400 ring-2 text-gray-900 rounded-sm h-10 w-10 p-1 mr-5"
      ...
      <div className="flex flex-row gap-5 justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 m-3 gap-3">
          {suggestedTopics.map((topic) => (
            <div
              className="group bg-white border-2 w-50 h-50 border-gray-100 aspect-square p-2 rounded-sm shadow-lg flex flex-col justify-between"
              key={topic.id}
            >
              <div>
                <h4 className="group-hover:text-yellow-400 break-words whitespace-normal">
                  {topic.title}
                </h4>
              ...
              {topic.author.nickname === userName && (
                <button
                  className="text-red-500 p-1 rounded-sm cursor-pointer self-end text-xs hover:bg-red-500 hover:text-white"
                  onClick={() => addOnClick()}
                >
            ...
            <div className="group bg-white border-2 w-50 h-50 border-gray-100 p-2 rounded-sm shadow-lg flex flex-col justify-between">
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="bg-transparent outline-none font-medium text-green-800 resize-none"
                  value={title}
                  onChange={(e) => {
                    setError("");
                ...
                <textarea
                  ...
                  className="min-h-[60px] bg-transparent outline-none resize-none  text-xs overfow-hidden"
                  rows={6}
                  value={description}
                  maxLength={200}
                  onChange={(e) => {
              ...
              <div className="flex flex-col gap-2 justify-between">
                {error ? (
                  <p className="text-red-500 text-center py-2 text-xs">
                    {error}
                  </p>
                ) : (
                  <button
                    className="bg-green-400 hover:bg-green-500 self-right text-white rounded-sm py-1 px-2 flex flex-row items-center gap-1 w-fit self-end text-xs"
                    onClick={() => addOnClick()}
                  >
                    <Check size={15} />
                    Add
  ...
  return (
    <div className="h-[35rem] w-[70%] border-2 border-gray-100 rounded-sm z-index-2 bg-white shadow-xl px-10 py-5 flex flex-col justify-between gap-2">
      ...
      <div className="flex justify-end">
        <p className="bg-white hover:text-yellow-400 hover:bg-yellow-50 ring-gray-400 hover:ring-yellow-300 ring-2 text-gray-900 rounded-sm p-2">
          Publish
        </p>
      </div>
    </div>
  ...
  const sections = [
    {
      ...
      name: "Suggested",
      description:
        "Members might suggest topics for other to write about in their letters. Here you can share and find inspiration",
      Icon: "PencilLine",
    },
    {
      id: 1,
      name: "Letterboard",
      description:
        "Share letters with the whole community! Anyone can read it or take it and correct it",
      Icon: "MessagesSquare",
    },
  ];
    ...
    <div className="md:w-[30%] flex flex-row md:flex-col gap-2">
      {sections.map((section) => {
        ...
        return (
          ...
          >
            <p
              className="flex rounded-l-lg bg-gray-200 shadow-md w-[20%] text-2xl items-center justify-center align-middle py-10 items-center
                bg-orange-300"
            >
            ...
            <div
              className={`px-8 py-4 rounded-r-lg bg-gray-50 shadow-md w-full max-w-5xl text-black border-2 ${section.id === sectionSelected && " border-orange-200"}`}
            >
