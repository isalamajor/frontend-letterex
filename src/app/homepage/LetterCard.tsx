...
const LetterCard: React.FC<LetterCardProps> = ({
  ...
  // Warm up the dynamic edit route so navigation feels immediate.
  useEffect(() => {
    router.prefetch(`/edit-letter/${id}`);
  }, [router, id]);
  ...
  const goToCorrection =
    ...
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      router.push(`/view-correction/${correctedLetterId}`);
    };
  ...
  return (
    <div className="block w-full relative group my-1">
      <div
        className={`cursor-pointer px-8 py-4 rounded-lg bg-gray-50 dark:bg-neutral-850 shadow-md relative h-[12vh]
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <p className="font-semibold text-gray-800 dark:text-gray mb-2 highlighted-text">
            {diary}
          </p>
        </div>
      ...
      <div
        className={`
          absolute h-full w-[17%] top-1/2 right-0 transform -translate-y-1/2 bg-gray-50 dark:bg-neutral-850 text-black dark:text-gray-200 px-4 py-2 mr-3 ml-0 rounded-lg
      ...
      >
        <div className="flex flex-col gap-2 px-2">
          {deleteMode ? (
            <>
              <p className="text-xs dark:text-red-100">Select to delete</p>
              <input
                type="checkbox"
                checked={isSelected}
                onChange={() => {
                ...
                className=" inset-0 m-auto w-6 h-6 bg-gray-100 dark:bg-neutral-800 border-gray-300 dark:border-neutral-600 rounded focus:ring-red-500 focus:ring-1 accent-red-500"
              />
            </>
          ) : sharedWith.length > 0 ? (
            sharedWith.map((user, index) => {
              ...
              return (
                <div
                  className="dark:text-gray-100 inline-block px-2"
                  key={`${btnId}-div`}
                >
                  <button
                  ...
                  >
                    {user.correctionSentBack ? (
                      ...
                      <div className="text-sm flex flex-row gap-x-1 max-w-[100px] truncate overflow-hidden whitespace-nowrap dark:text-gray-900">
                        <p>⏰</p>
                        <p>{user.nickname}</p>
                      </div>
                    )}
